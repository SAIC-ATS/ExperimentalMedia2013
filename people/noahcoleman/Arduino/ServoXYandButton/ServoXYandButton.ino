...
const int ledPin = 13;  //led will show on Arduino when mouse is clicked
...
const int NUM_SERVO_PINS = NUM_BYTES_EXPECTED; // we expect one byte for each servo
const byte SERVO_PINS[NUM_SERVO_PINS] = { 9, 10 };//array with pin numbers for servos
byte servoValues[NUM_SERVO_PINS] = { 0, 0 };
...
void loop()
{
 ...
 int i;
 if (servoValues[i] > 0 || servoValues[i] < 180)
 {
   ...
   //I need to split the incoming array into the x and y value 
 //and the ledOn/Off value.
 

  // write our values to the output.
  for(int i = 0; i < NUM_SERVO_PINS; ++i)
  {
     analogWrite(SERVO_PINS[i],servoValues[i]); 
