    ...
    while(serial.available() > 0)
    {
        ...
        else if(myByte == '\n')
        {
          ...
          //  potValue = ofToInt(buffer);
            result = ofSplitString(buffer, ",");
            potValue1 = ofToInt(result[0]);
            potValue2 = ofToInt(result[1]);
...
void ofApp::draw()
{
    int mValue1 = ofMap(potValue1,0,1024,0,255);
    int mValue2 = ofMap(potValue1,0,1024,0,255);
    ofBackground(mValue1);
 ...
 //   int backgroundColor = ofMap(potValue1,0,1024,0,255);
  
    ofSetColor(potValue1, potValue2, 0);
    ofCircle(potValue1, potValue2, 100);
